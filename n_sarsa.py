...
# Training Parameters
J = 3           # overridden by command line arguments; determines which parameter index to use
N              = [1, 3, 6, 9, 12, 15]
...
class Agent:
    ...
    def gameover(self, score):
        ...
        if len(self.score_hist) >= config.EPISODES_PER_SEQUENCE:
            ...
            self.Q = torch.ones((NUM_Y_STATES, NUM_V_STATES, NUM_DX_STATES, NUM_PIPE_STATES, NUM_ACTIONS + 1)) * 3
            self.Q[:,:,:,:,1] /= 1.01    # gives no-flap more value
            self.Q[:,:,:,:,2] = 0.1     # certainty, initialized to 20%, reduced as points are awarded
            self.score_hist = []
