...
class TDNSarsa:
    def __init__(self, N=13, speedup_factor=1, xpos=57, ypos_min=0, ypos_max=100,
                 yvel_min=-10, yvel_max=10, dx_max=50, pipec_min=0, pipec_max=100,
                 step_size=0.4, discount=0.8):
    ...
    def move(self, y_pos, y_vel, upipes, lpipes):
        ...
        if d_state > threshold:
            ...
            action = self.sarsa_onpolicy_tdn()

        return flap if action else no_flap

    def sarsa_onpolicy_tdn(self) -> bool:
        # State
        ...
        self.q_state_action_epsilon[state_t1][2] /= 1.1
        action_t1 = int(epsilon_greedy(q_noflap=q_noflap, q_flap=q_flap, epsilon=epsilon))
         # Reward
    ...
    def save(self):
        ...
        with open(f"tdn/flappy_sarsa-{d1}", 'wb') as f:
    ...
    def make_vectors(self):
        if READ_CACHE:
            try:
                ...
                names = listdir("tdn")  # read most recent list
                name = names[len(names) - 1]
                with open(f"tdn/{name}", 'rb') as f:
        ...
        self.q_state_action_epsilon = np.insert(
            arr=np.random.uniform(low=-2000.0, high=-200.0, size=(NUM_STATES, 2)),
            obj=2,
            values=np.ones(NUM_STATES) * 0.5,  # epsilon
            axis=1)
